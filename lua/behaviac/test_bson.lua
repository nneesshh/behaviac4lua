bson=require'behaviac.external.bson'
epoch=os.time({year=1999, month=1, day=1, hour=0})
...
bsondoc2=bson.encode{username="maroc", 
		     info={first="todd",
			     last="coram",
			     age=46,
			     signature1=bson.binary("\003\001\002\003",bson.B_GENERIC),
			     signature2=bson.binary("\004\001\002\003"),
				 saved=true,
				 epoch=bson.utc_datetime(epoch),
...
local f=io.open("./lua/data/test.bson", "wb")
f:write(bsondoc1)
f:write(bsondoc2)
f:write(bsondoc3)
f:close()
...
local f2=io.open("./lua/data/test.bson", "rb")
while true do
   local btab = bson.decode_next_io(f2)
